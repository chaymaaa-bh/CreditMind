...
feat_fac['jours_depuis_derniere_fac'] = (reference_date - feat_fac['date_derniere_facture']).dt.days
...
clients['solde_restant']          = (clients['montant_ttc_total'] - clients['montant_reg_total']).round(3)

# Fix: clients with 1 invoice have NaN std → replace with 0
clients['montant_ttc_std'] = clients['montant_ttc_std'].fillna(0)

# ─── 5. LABEL DE SOLVABILITÉ ──────────────────────────────────────────────────
# 1 = à risque si : jamais payé OU retard max > 30j OU > 50% paiements en retard OU encaissement < 50%
clients['label_risque'] = (
    (clients['nb_reglements']      == 0) |
    (clients['retard_max_jours']   >  30) |
    (clients['taux_retard']        >  0.5) |
    (clients['ratio_encaissement'] <  0.5)
).astype(int)
...
for col in ['montant_ttc_total','montant_ttc_moyen','montant_ttc_max','montant_ttc_std',
            ...
            'ratio_avoirs','taux_retard','ratio_encaissement','solde_restant']:
...
cols = [
    ...
    'mode_paiement_code','nb_modes_distincts','ratio_encaissement',
    'solde_restant','label_risque',
]
...
print(f"  Solde restant moyen  : {clients_final['solde_restant'].mean():,.0f} DT")
