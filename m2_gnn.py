...
try:
    ...
    try:
        ...
        class SimpleGNN(nn.Module):
            ...
            def aggregate_neighbors(self, x, src_t, dst_t, n_nodes):
                """Agrège les features des voisins (vectorisé)"""
                feat_dim = x.shape[1]
                agg = torch.zeros(n_nodes, feat_dim)
                agg.index_add_(0, dst_t, x[src_t])
                count = torch.zeros(n_nodes, 1)
                count.index_add_(0, dst_t, torch.ones(len(src_t), 1))
                count = count.clamp(min=1)
                return agg / count
 
            def forward(self, x, src_t, dst_t):
                n = x.shape[0]
                neigh = self.aggregate_neighbors(x, src_t, dst_t, n)
    ...
    for epoch in range(1, 201):
        ...
        if USE_PYGEOMETRIC:
            ...
            out = model(X_tensor, src_t, dst_t)
        ...
        if epoch % 20 == 0:
            ...
            with torch.no_grad():
                if USE_PYGEOMETRIC:
                    ...
                    logits = model(X_tensor, src_t, dst_t)
                ...
                try:
                    ...
                    acc = accuracy_score(y[tm], preds[tm])
                    f1  = f1_score(y[tm], preds[tm], zero_division=0)
                    if auc > best_auc:
                        best_auc = auc
                    print(f"  Epoch {epoch:3d} | Loss: {loss.item():.4f} | AUC: {auc:.4f} | Acc: {acc:.4f} | F1: {f1:.4f}")
                except:
    ...
    tm = test_mask.numpy()
    final_preds = (probs[tm] >= 0.5).astype(int)
    final_auc  = roc_auc_score(y[tm], probs[tm])
    final_f1   = f1_score(y[tm], final_preds, zero_division=0)
    final_prec = precision_score(y[tm], final_preds, zero_division=0)
    final_rec  = recall_score(y[tm], final_preds, zero_division=0)
